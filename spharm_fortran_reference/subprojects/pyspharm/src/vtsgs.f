...
c
  ...
  950 do 14 k=1,nt
   ...
   65 continue
      ...
      subroutine vtsgs_dbg_open()
      integer iu
      logical opened
      save iu, opened
      data iu /99/, opened /.false./
      if (.not. opened) then
         open(unit=iu,file='vtsgsi_debug.log',status='unknown')
         rewind iu
         write(99,'(A)') '===== RUN START VTSGSI DEBUG ====='
         opened = .true.
      endif
      return
      end

      subroutine vtsgs_dbg_sep(label)
      character*(*) label
      call vtsgs_dbg_open()
      write(99,'(A)') '==============='
      write(99,'(A)') label
      ...
      subroutine vtsgs_dbg_vec_r8(label,arr,n)
      character*(*) label
      integer n
      double precision arr(*)
      call vtsgs_dbg_open()
      if (n .le. 0) then
         write(99,'(A,A)') label, ' size=0'
      else if (n .eq. 1) then
         write(99,'(A,A,I8,A,ES24.16)') label,
     1      ' size=', n, ' only=', arr(1)
      else
         write(99,'(A,A,I8,A,ES24.16,A,ES24.16)') label,
     1      ' size=', n, ' head=', arr(1), ' tail=', arr(n)
      endif
      return
      end

      subroutine vtsgs_dbg_vec_r4(label,arr,n)
      character*(*) label
      integer n
      real arr(*)
      call vtsgs_dbg_open()
     ...
     1      ' size=', n, ' head=', arr(1), ' tail=', arr(n)
      ...
      subroutine vtsgs_dbg_vin_plane(label,vin,imid,nlat,iplane)
      character*(*) label
      integer imid,nlat,iplane
      real vin(imid,nlat,3)
      call vtsgs_dbg_open()
      if (imid .le. 0 .or. nlat .le. 0) then
         write(99,'(A,A,I4)') label, ' plane=', iplane
         write(99,'(A)') '  empty plane'
      else
         write(99,'(A,A,I4,A,2I8)') label, ' plane=', iplane,
     1      ' shape=', imid, nlat
         write(99,'(A,2I8,A,ES24.16)') '  first(np1,i)=', 1, 1,
     1      ' value=', vin(1,1,iplane)
         write(99,'(A,2I8,A,ES24.16)') '  last(np1,i)=', nlat, imid,
     1      ' value=', vin(imid,nlat,iplane)
      endif
      return
      end

      subroutine vtsgs_dbg_vin_plane_window(label,vin,imid,nlat,iplane)
      character*(*) label
      integer imid,nlat,iplane,i1,i2,np1a,np1b
      real vin(imid,nlat,3)
      call vtsgs_dbg_open()
      write(99,'(A,A,I4,A,2I8)') label, ' plane=', iplane,
     1   ' shape=', imid, nlat
      ...
      i1 = min0(4,imid)
      np1a = min0(4,nlat)
      write(99,'(A)') '  head window:'
      do 20 np1b=1,np1a
         do 10 i2=1,i1
            write(99,'(A,2I8,A,ES24.16)') '    idx=', np1b, i2,
     1         ' value=', vin(i2,np1b,iplane)
   10    continue
   20 continue
      i1 = max0(1,imid-3)
      np1a = max0(1,nlat-3)
      write(99,'(A)') '  tail window:'
      do 40 np1b=np1a,nlat
         do 30 i2=i1,imid
            write(99,'(A,2I8,A,ES24.16)') '    idx=', np1b, i2,
     1         ' value=', vin(i2,np1b,iplane)
   30    continue
   40 continue
      ...
      subroutine vtsgs_dbg_vec_r4_window(label,arr,n)
      character*(*) label
      integer n,i1,i2
      real arr(*)
      call vtsgs_dbg_open()
      write(99,'(A,A,I8)') label, ' size=', n
      if (n .le. 0) return
      i1 = min0(8,n)
      write(99,'(A)') '  head window:'
      do 10 i2=1,i1
         write(99,'(A,I8,A,ES24.16)') '    idx=', i2,
     1      ' value=', arr(i2)
   10 continue
      i1 = max0(1,n-7)
      write(99,'(A)') '  tail window:'
   ...
   20 continue
      ...
      subroutine vtsgs_dbg_vec_r4_nonzero(label,arr,n)
      character*(*) label
      integer n,i,nnz,first_nz,last_nz
      real arr(*)
      call vtsgs_dbg_open()
      nnz = 0
      first_nz = 0
      ...
      do 10 i=1,n
         if (arr(i) .ne. 0.) then
            nnz = nnz+1
            if (first_nz .eq. 0) first_nz = i
            last_nz = i
         endif
   10 continue
      write(99,'(A,A,I8,A,I8,A,I8)') label,
     1   ' nnz=', nnz, ' first_nz=', first_nz, ' last_nz=', last_nz
      return
      end
...
c
      ...
      ierror = 0
      call vtsgs_dbg_sep('ENTER VTSGSI')
      write(99,'(A,1X,I8)') 'nlat', nlat
      write(99,'(A,1X,I8)') 'nlon', nlon
      write(99,'(A,1X,I8)') 'mmax', mmax
      write(99,'(A,1X,I8)') 'imid', imid
      write(99,'(A,1X,I8)') 'lzimn', lzimn
      write(99,'(A,1X,I8)') 'labc', labc
      write(99,'(A,1X,I8)') 'lvin', lvin
      write(99,'(A,1X,I8)') 'lwvbin', lwvbin
      write(99,'(A,1X,I8)') 'ltheta', ltheta
      iw1 = lvin+1
      iw2 = iw1+lwvbin
      jw1 = nlat+1
      jw2 = jw1+nlat
     ...
     1              DWORK,DWORK(JW1),DWORK(JW2),IERROR)
      ...
      call vtsgs_dbg_vec_r4('wvts_vb_block',wvts,lzimn)
      call vtsgs_dbg_vec_r4('wvts_wb_block',wvts(lzimn+1),lzimn)
      call vtsgs_dbg_vec_r4('wvts_hrffti_block',wvts(2*lzimn+1),nlon+15)
      return
      end
     ...
     1                       theta,wts,dwork,ierror)
      ...
      lwvbin = 2*nlat*imid+labc+labc+labc
      do 7 j=1,3
      do 7 np1=1,nlat
      do 7 i=1,imid
      vin(i,np1,j) = 0.
    7 continue
      do 8 mn=1,(mmax*(nlat+nlat-mmax+1))/2
      do 8 i=1,imid
      vb(i,mn) = 0.
      wb(i,mn) = 0.
    8 continue
    ...
    9 continue
      ...
      ierror = 10+ierr
      return      
   10 call vtsgs_dbg_sep('ENTER VETG1')
      write(99,'(A,1X,I8)') 'nlat', nlat
      write(99,'(A,1X,I8)') 'nlon', nlon
      write(99,'(A,1X,I8)') 'imid', imid
      write(99,'(A,1X,I8)') 'mmax', mmax
      call vtsgs_dbg_vec_r8('theta',theta,nlat)
      call vtgint (nlat,nlon,theta,wvbin,dwork)
      call vtsgs_dbg_vec_r4('wvbin_after_vtgint',wvbin,
     1  2*nlat*imid+(max0(mmax-2,0)*(nlat+nlat-mmax-1))/2)
      do 33 mp1=1,mmax
      m = mp1-1
      call vbin (0,nlat,nlon,m,vin,i3,wvbin)
      if (nlat .le. 8 .or. mp1 .le. 3 .or. mp1 .ge. mmax-1) then
         write(99,'(A,1X,I8,1X,I8,1X,I8)')
     1      'AFTER_VBIN mp1 m i3', mp1, m, i3
         call vtsgs_dbg_vin_plane('vin_after_vbin',vin,imid,nlat,i3)
         call vtsgs_dbg_vin_plane_window('vin_after_vbin_window',vin,
     1      imid,nlat,i3)
      endif
      do 33 np1=mp1,nlat
   ...
   33 continue
      ...
      call vtsgs_dbg_vec_r4('wvbin_after_wtgint',wvbin,
     1  2*nlat*imid+(max0(mmax-2,0)*(nlat+nlat-mmax-1))/2)
      do 34 mp1=1,mmax
      m = mp1-1
      ...
      if (nlat .le. 8 .or. mp1 .le. 3 .or. mp1 .ge. mmax-1) then
         write(99,'(A,1X,I8,1X,I8,1X,I8)')
     1      'AFTER_WBIN mp1 m i3', mp1, m, i3
         call vtsgs_dbg_vin_plane('vin_after_wbin',vin,imid,nlat,i3)
         call vtsgs_dbg_vin_plane_window('vin_after_wbin_window',vin,
     1      imid,nlat,i3)
      endif
      do 34 np1=mp1,nlat
      mn = m*(nlat-1)-(m*(m-1))/2+np1
   ...
   34 continue
      call vtsgs_dbg_vec_r4('vb_final_linear',vb,imid*
     1     (mmax*(nlat+nlat-mmax+1)/2))
      call vtsgs_dbg_vec_r4_window('vb_final_linear_window',vb,imid*
     1     (mmax*(nlat+nlat-mmax+1)/2))
      call vtsgs_dbg_vec_r4_nonzero('vb_final_linear_stats',vb,imid*
     1     (mmax*(nlat+nlat-mmax+1)/2))
      call vtsgs_dbg_vec_r4('wb_final_linear',wb,imid*
     1     (mmax*(nlat+nlat-mmax+1)/2))
      call vtsgs_dbg_vec_r4_window('wb_final_linear_window',wb,imid*
     1     (mmax*(nlat+nlat-mmax+1)/2))
      call vtsgs_dbg_vec_r4_nonzero('wb_final_linear_stats',wb,imid*
     1     (mmax*(nlat+nlat-mmax+1)/2))
      return
      end
