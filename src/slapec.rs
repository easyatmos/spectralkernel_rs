...
/// Core Rust implementation of `slapec`.
///
/// # Parameters
...
pub fn slapec_impl(
    ...
    let l2 = imid;
    let lwmin = 2 * nlat * l2 + 3 * ((l1.saturating_sub(2)) * (2 * nlat - l1 - 1)) / 2 + nlon + 15;
    ierror = 9;
    if wshsec.len() < lwmin {
        return Ok((Vec::new(), ierror));
    }
    ...
    let lwkmin = if isym == 0 {
        nlat * (2 * nt * nlon + (6 * l2).max(nlon) + 2 * nt * l1 + 1)
    } else {
        l2 * (2 * nt * nlon + (6 * nlat).max(nlon)) + nlat * (2 * nt * l1 + 1)
    };
    ierror = 10;
    if lwork < lwkmin {
        return Ok((Vec::new(), ierror));
    ...
    let lwork_shsec = lwork.saturating_sub(2 * mn + nlat);
    shsec_impl(&alap, &blap, nlat, nt, isym, wshsec, lwork_shsec)
}

#[pyfunction]
/// Rust entry point for `slapec`.
///
/// # Parameters
/// - `nlon`: Number of longitudes in the grid.
/// - `a`: Cosine spectral coefficients in scalar layout.
...
pub fn slapec<'py>(
    ...
    if ashape.len() != 2 && ashape.len() != 3 {
        return Err(PyValueError::new_err("slapec expects rank-2 or rank-3 a/b"));
    }
