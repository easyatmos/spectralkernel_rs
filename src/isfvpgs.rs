...
/// Core Rust implementation of `isfvpgs`.
///
/// # Parameters
...
pub fn isfvpgs_impl(
    ...
    let base_lvhsgs = 2 * l2 * lmn + nlon + 15;
    ierror = 9;
    if wvhsgs.len() < base_lvhsgs {
        return Ok((Vec::new(), Vec::new(), ierror));
    }
...
pub fn isfvpgs<'py>(
    ...
    isfvpgs_isym(py, nlon, 0, as_, bs, av, bv, wvhsgs, lwork)
}

#[pyfunction]
/// Rust entry point for `isfvpgs_isym`.
///
/// # Parameters
/// - `nlon`: Number of longitudes in the grid.
/// - `isym`: Symmetry selector used by Legendre tables.
...
pub fn isfvpgs_isym<'py>(
    ...
    if shape.len() != 2 && shape.len() != 3 {
        return Err(PyValueError::new_err(
            "isfvpgs expects rank-2 or rank-3 coefficient arrays",
        ));
    }

    let nlat = shape[0];
    let nt = if shape.len() == 2 { 1 } else { shape[2] };
    let (v, w, ierror) = isfvpgs_impl(
        nlon,
    ...
    let v = expand_isfvpgs_output(v, nlat, nlon, nt, isym);
    let w = expand_isfvpgs_output(w, nlat, nlon, nt, isym);
