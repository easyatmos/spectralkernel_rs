...
def calc_lwork_gradgs(nlat: int, nlon: int, nt: int, isym: int):
    l1 = min(nlat, nlon // 2 if nlon % 2 == 0 else (nlon + 1) // 2)
    l2 = nlat // 2 if nlat % 2 == 0 else (nlat + 1) // 2
    if isym == 0:
        return nlat * ((2 * nt + 1) * nlon + 2 * l1 * nt + 1)
    return (2 * nt + 1) * l2 * nlon + nlat * (2 * l1 * nt + 1)
...
def run_case(nlat: int, nlon: int, nt: int, isym: int = 0):
    ...
    lwork_gradgs = calc_lwork_gradgs(nlat, nlon, nt, isym)
    if isym == 0:
        v_f, w_f, ierr_f = fort_sp.gradgs(nlon, a, b, np.asarray(wvhsgs, dtype=np.float32), lwork_gradgs)
        v_r, w_r, ierr_r = rust_sp.gradgs(a, b, np.asarray(wvhsgs, dtype=np.float32), lwork_gradgs)
    else:
        v_f, w_f, ierr_f = fort_sp.gradgs(nlon, a, b, np.asarray(wvhsgs, dtype=np.float32), lwork_gradgs, isym=isym)
        v_r, w_r, ierr_r = rust_sp.gradgs_isym(a, b, isym, np.asarray(wvhsgs, dtype=np.float32), lwork_gradgs)

    assert ierr_f == 0, ("fortran gradgs failed", nlat, nlon, nt, isym, ierr_f)
    assert ierr_r == 0, ("rust gradgs failed", nlat, nlon, nt, isym, ierr_r)

    print(f"\n{'=' * 80}\ngradgs: nlat={nlat}, nlon={nlon}, nt={nt}, isym={isym}\n{'=' * 80}")
    summarize_diff("gradgs v: fortran vs rust", v_f, v_r)
    summarize_diff("gradgs w: fortran vs rust", w_f, w_r)
