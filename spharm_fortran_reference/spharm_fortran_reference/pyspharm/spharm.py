...
# gaussian computed legendre branch can call spherepack gradient directly.

        if self.gridtype == 'gaussian' and self.legfunc == 'computed':
            nlat = self.nlat
            nlon = self.nlon
            if nlat%2:                              # nlat is odd
                n2 = (nlat + 1)/2
                ...
                n2 = nlat/2
            lwork = nlat*(2*nt*nlon+max(6*n2,nlon)+2*nlat*nt+1)
            a, b = _spherepack.onedtotwod(chispec, nlat)
            v, w, ierror = _spherepack.gradgc(nlon, a, b, self.wvhsgc, lwork)
            if ierror != 0:
                msg = 'In return from call to gradgc in Spharmt.getgrad ierror =  %d' % ierror
                raise ValueError(msg)
            uchi = w/self.rsphere
            vchi = -v/self.rsphere
        else:
