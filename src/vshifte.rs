...
fn vhftoff_impl(nlon: usize, nlat: usize, off: &[f32], wsav: &[f32]) -> PyResult<Vec<f32>> {
    let n2 = (nlon + 1) / 2;
    let nlat2 = nlat + nlat;
    let nlatp1 = nlat + 1;
    let mut reg = vec![0.0_f32; nlon * nlatp1];
    let lat_wsav = &wsav[..(4 * nlat + 16)];
    let lon_offset = 4 * nlat + 16;
    let lon_wsav = &wsav[lon_offset..];

    if !nlon.is_multiple_of(2) {
        let mut rlou = vec![0.0_f32; nlat * nlon];
        for i in 0..nlat {
            for j in 0..nlon {
                rlou[idx_row(nlon, i, j)] = off[idx_geo(nlat, j, i)];
            }
        }
        let rlou = shift_rows_real(&rlou, nlat, nlon, lon_wsav)?;

        let mut rlatu = vec![0.0_f32; nlon * nlat2];
        for j in 0..(n2 - 1) {
            let js = j + n2;
            for i in 0..nlat {
                rlatu[idx_row(nlat2, j, i)] = off[idx_geo(nlat, j, i)];
                rlatu[idx_row(nlat2, j, nlat + i)] = -rlou[idx_row(nlon, nlat - 1 - i, js)];
            }
        }
        for j in (n2 - 1)..nlon {
            let js = j - (n2 - 1);
        ...
        let rlatu = shift_rows_real(&rlatu, nlon, nlat2, lat_wsav)?;
        for j in 0..nlon {
            for i in 0..nlatp1 {
                reg[idx_reg(nlatp1, j, i)] = rlatu[idx_row(nlat2, j, i)];
            }
        }
    } else {
        let mut rlatu = vec![0.0_f32; n2 * nlat2];
        for j in 0..n2 {
            let js = n2 + j;
            for i in 0..nlat {
                rlatu[idx_row(nlat2, j, i)] = off[idx_geo(nlat, j, i)];
                rlatu[idx_row(nlat2, j, nlat + i)] = -off[idx_geo(nlat, js, nlat - 1 - i)];
            }
        }

        let rlatu = shift_rows_real(&rlatu, n2, nlat2, lat_wsav)?;
        for j in 0..n2 {
            let js = n2 + j;
            reg[idx_reg(nlatp1, j, 0)] = rlatu[idx_row(nlat2, j, 0)];
            reg[idx_reg(nlatp1, js, 0)] = -rlatu[idx_row(nlat2, j, 0)];
            for i in 1..nlatp1 {
                reg[idx_reg(nlatp1, j, i)] = rlatu[idx_row(nlat2, j, i)];
                reg[idx_reg(nlatp1, js, i)] = -rlatu[idx_row(nlat2, j, nlat2 - i)];
            }
        }
    }

    let mut rlonu = vec![0.0_f32; nlatp1 * nlon];
    for j in 0..nlon {
        for i in 0..nlatp1 {
            rlonu[idx_row(nlon, i, j)] = reg[idx_reg(nlatp1, j, i)];
        }
    }

    let rlonu = shift_rows_real(&rlonu, nlatp1, nlon, lon_wsav)?;

    for j in 0..nlon {
        for i in 0..nlatp1 {
            reg[idx_reg(nlatp1, j, i)] = rlonu[idx_row(nlon, i, j)];
        }
    }
...
fn vhftreg_impl(nlon: usize, nlat: usize, reg: &[f32], wsav: &[f32]) -> PyResult<Vec<f32>> {
    let n2 = (nlon + 1) / 2;
    let nlat2 = nlat + nlat;
    let nlatp1 = nlat + 1;
    let mut off = vec![0.0_f32; nlon * nlat];
    let lat_wsav = &wsav[..(4 * nlat + 16)];
    let lon_offset = 4 * nlat + 16;
    let lon_wsav = &wsav[lon_offset..];
    ...
    if !nlon.is_multiple_of(2) {
        ...
        for j in 0..n2 {
            let js = j + n2 - 1;
            rlatu[idx_row(nlat2, j, 0)] = reg[idx_reg(nlatp1, j, 0)];
            for i in 1..nlat {
                rlatu[idx_row(nlat2, j, i)] = reg[idx_reg(nlatp1, j, i)];
                rlatu[idx_row(nlat2, j, nlat + i)] = -rlonu[idx_row(nlon, nlat - i, js)];
            }
            rlatu[idx_row(nlat2, j, nlat)] = reg[idx_reg(nlatp1, j, nlat)];
        }
        for j in n2..nlon {
            let js = j - n2;
            rlatu[idx_row(nlat2, j, 0)] = reg[idx_reg(nlatp1, j, 0)];
        ...
        for j in 0..nlon {
            for i in 0..nlat {
                off[idx_geo(nlat, j, i)] = rlatu[idx_row(nlat2, j, i)];
            }
        }
    } else {
        let mut rlatu = vec![0.0_f32; n2 * nlat2];
        for j in 0..n2 {
            ...
            for i in 1..nlat {
                rlatu[idx_row(nlat2, j, i)] = reg[idx_reg(nlatp1, j, i)];
                rlatu[idx_row(nlat2, j, nlat + i)] = -reg[idx_reg(nlatp1, js, nlat - i)];
            }
            rlatu[idx_row(nlat2, j, nlat)] = reg[idx_reg(nlatp1, j, nlat)];
        }
        ...
        for j in 0..n2 {
            ...
            for i in 0..nlat {
                off[idx_geo(nlat, j, i)] = rlatu[idx_row(nlat2, j, i)];
                off[idx_geo(nlat, js, i)] = -rlatu[idx_row(nlat2, j, nlat2 - 1 - i)];
            }
        }
    }
    ...
    for j in 0..nlon {
        for i in 0..nlat {
            off[idx_geo(nlat, j, i)] = rlou[idx_row(nlon, i, j)];
        }
    }
...
pub fn vshifte_impl(
    ...
    if ioff == 0 {
        if u_input.len() != nlon * nlat || v_input.len() != nlon * nlat {
            return Err(PyValueError::new_err(
                "vshifte ioff=0 expects u and v shape (nlon, nlat)",
            ));
        }
        Ok((
            vhftoff_impl(nlon, nlat, u_input, wsav)?,
            vhftoff_impl(nlon, nlat, v_input, wsav)?,
            0,
        ))
    } else {
        if u_input.len() != nlon * (nlat + 1) || v_input.len() != nlon * (nlat + 1) {
            return Err(PyValueError::new_err(
                "vshifte ioff=1 expects u and v shape (nlon, nlat+1)",
            ));
        }
        Ok((
            vhftreg_impl(nlon, nlat, u_input, wsav)?,
            vhftreg_impl(nlon, nlat, v_input, wsav)?,
            0,
        ))
    }
}
...
pub fn vshifti<'py>(
    ...
    let (wsav, ier) = vshifti_impl(ioff, nlon, nlat, lsav);
...
pub fn vshifte<'py>(
    ...
    if u_shape.len() != 2 || v_shape.len() != 2 {
        return Err(PyValueError::new_err(
            "vshifte expects rank-2 u and v arrays",
        ));
    }
    if u_shape != v_shape {
        return Err(PyValueError::new_err(
            "vshifte expects u and v to have the same shape",
        ));
    }
    let nlon = u_shape[0];
    let nlat = if ioff == 0 {
        ...
        u_shape[1].checked_sub(1).ok_or_else(|| {
            PyValueError::new_err("vshifte ioff=1 expects second dimension at least 2")
        })?
    };

    let u_input = u.as_array().as_standard_layout().to_owned();
    let v_input = v.as_array().as_standard_layout().to_owned();
    let wsav_arr = wsav.as_array().as_standard_layout().to_owned();
    let (u_shifted, v_shifted, ier) = vshifte_impl(
        ioff,
        u_input
            .as_slice()
            .ok_or_else(|| PyValueError::new_err("u is not standard-layout after copy"))?,
        v_input
            .as_slice()
            .ok_or_else(|| PyValueError::new_err("v is not standard-layout after copy"))?,
        nlon,
        nlat,
        wsav_arr
            .as_slice()
    ...
    let u_out = ArrayD::from_shape_vec(IxDyn(&out_shape), u_shifted)
        .map_err(|err| PyValueError::new_err(err.to_string()))?;
    let v_out = ArrayD::from_shape_vec(IxDyn(&out_shape), v_shifted)
        .map_err(|err| PyValueError::new_err(err.to_string()))?;
    Ok((
        u_out.into_pyarray(py).into_any().unbind(),
        v_out.into_pyarray(py).into_any().unbind(),
        ier,
    ))
}
