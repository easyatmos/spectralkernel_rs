...
/// Core Rust implementation of `sfvpes`.
///
/// # Parameters
...
pub fn sfvpes_impl(
    ...
    let lshses_min = lpimn + nlon + 15;
    ierror = 9;
    if wshses.len() < lshses_min {
        return Ok((Vec::new(), Vec::new(), ierror));
    }

    let mab = nlat.min(nlon / 2 + 1);
    let ls = if isym == 0 { nlat } else { imid };
    let lwkmin = ls * (nt + 1) * nlon + 2 * mab * nlat * nt + nlat;
    ierror = 10;
    if lwork < lwkmin {
        return Ok((Vec::new(), Vec::new(), ierror));
    }
    ...
    let (sf_full, ierr_sf) = shses_impl(&a, &b, nlat, nt, wshses, sh_lwork_eff)?;
    if ierr_sf != 0 {
    ...
    let (vp_full, ierr_vp) = shses_impl(&a, &b, nlat, nt, wshses, sh_lwork_eff)?;
    if ierr_vp != 0 {
        return Ok((Vec::new(), Vec::new(), ierr_vp));
    }
    Ok((sf_full, vp_full, 0))
}

#[pyfunction]
/// Rust entry point for `sfvpes`.
///
/// # Parameters
/// - `nlon`: Number of longitudes in the grid.
/// - `br`: First vector coefficient family in vector layout.
...
pub fn sfvpes<'py>(
    ...
    sfvpes_isym(py, nlon, 0, br, bi, cr, ci, wshses, lwork)
}

#[pyfunction]
/// Rust entry point for `sfvpes_isym`.
///
/// # Parameters
/// - `nlon`: Number of longitudes in the grid.
/// - `isym`: Symmetry selector used by Legendre tables.
...
pub fn sfvpes_isym<'py>(
    ...
    if shape.len() != 2 && shape.len() != 3 {
        return Err(PyValueError::new_err(
            "sfvpes expects rank-2 or rank-3 coefficient arrays",
        ));
    }
    let nlat = shape[0];
    let nt = if shape.len() == 2 { 1 } else { shape[2] };
    let (sf, vp, ierror) = sfvpes_impl(
        nlon,
