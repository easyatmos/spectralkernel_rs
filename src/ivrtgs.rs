...
/// Core Rust implementation of `ivrtgs`.
///
/// # Parameters
...
pub fn ivrtgs<'py>(
    ...
    if ashape.len() != 2 && ashape.len() != 3 {
        return Err(PyValueError::new_err("ivrtgs expects rank-2 or rank-3 a/b"));
    }
    let nlat = ashape[0];
    ...
    let (v, w, pertrb, _idvw, _nlon, ierror) =
        ivrtgs_impl(&abuf, &bbuf, nlat, nt, 0, wvhsgs.as_slice()?, lwork)?;
    if ierror != 0 {
...
#[pyfunction]
/// Rust entry point for `ivrtgs_isym`.
///
/// # Parameters
/// - `a`: Cosine spectral coefficients in scalar layout.
/// - `b`: Sine spectral coefficients in scalar layout.
...
pub fn ivrtgs_isym<'py>(
    ...
    if ashape.len() != 2 && ashape.len() != 3 {
        return Err(PyValueError::new_err(
            "ivrtgs_isym expects rank-2 or rank-3 a/b",
        ));
    }
    let nlat = ashape[0];
    let nt = if ashape.len() == 2 { 1 } else { ashape[2] };
    ...
    let (v, w, pertrb, _idv, _nlon, ierror) =
        ivrtgs_impl(&abuf, &bbuf, nlat, nt, isym, wvhsgs.as_slice()?, lwork)?;
    if ierror != 0 {
        let empty2 = ArrayD::<f32>::from_shape_vec(IxDyn(&[0, 0]), Vec::new())
            .map_err(|e| PyValueError::new_err(e.to_string()))?;
        let empty1 = ArrayD::<f32>::from_shape_vec(IxDyn(&[0]), Vec::new())
