...
def run_case(nlat: int, nlon: int, nt: int):
    lshsgs, init_lwork, ldwork = calc_shsgs_sizes(nlat, nlon)
    wshsgs, ierr0 = rust_sp.shsgsi(nlat, nlon, lshsgs, init_lwork, ldwork)
    assert ierr0 == 0
    a, b = make_coeffs(nlat, nt)
    lwork = nlat * nlon * (nt + 1)
    g_f, ierr_f = fort_sp.shsgs(nlon, a, b, np.asarray(wshsgs, dtype=np.float32), lwork)
    g_r, ierr_r = rust_sp.shsgs(a, b, np.asarray(wshsgs, dtype=np.float32), lwork)
    print(f"\n{'=' * 80}\nshsgs: nlat={nlat}, nlon={nlon}, nt={nt}\n{'=' * 80}")
    print("ierror fortran =", ierr_f)
    print("ierror rust    =", ierr_r)
    summarize_diff("shsgs g", g_f, g_r)
