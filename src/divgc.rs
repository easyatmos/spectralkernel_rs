...
pub fn divgc_impl(
    ...
    let l2 = imid;
    let lwmin_saved_i64 = (nlat as i64) * (2 * l2 as i64 + 3 * l1 as i64 - 2)
        + (3 * l1 as i64 * (1 - l1 as i64)) / 2
        + nlon as i64
        + 15;
    let lwmin_saved = usize::try_from(lwmin_saved_i64)
        .map_err(|_| PyValueError::new_err("invalid lshsgc minimum length computation"))?;
    ierror = 9;
    if wshsgc.len() < lwmin_saved {
        return Ok((Vec::new(), ierror));
    }
...
pub fn divgc<'py>(
    ...
    if brshape.len() != 2 && brshape.len() != 3 {
        return Err(PyValueError::new_err(
            "divgc expects rank-2 or rank-3 br/bi",
        ));
    }
