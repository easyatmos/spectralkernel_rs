...
pub fn ivlapes_impl(
    ...
    let lzimn = idz * imid;
    let need_lvhses = 2 * lzimn + nlon + 15;
    ierror = 9;
    ...
    let lwkmin = if ityp <= 2 {
        ...
        (2 * nt + 1) * imid * nlon + nlat * (4 * nt * l1 + 1)
    };
    ierror = 10;
    if lwork < lwkmin {
        return Ok((Vec::new(), Vec::new(), ierror));
    ...
    let lwork_vhses = if matches!(ityp, 0 | 3 | 6) {
...
pub fn ivlapes<'py>(
    ...
    ivlapes_ityp(py, nlon, br, bi, cr, ci, 0, wvhses, lwork)
}
...
pub fn ivlapes_ityp<'py>(
    ...
    if shape.len() != 2 && shape.len() != 3 {
        return Err(PyValueError::new_err(
            "ivlapes expects rank-2 or rank-3 coefficient arrays",
        ));
    }

    let nlat = shape[0];
    let nt = if shape.len() == 2 { 1 } else { shape[2] };
    let (v, w, ierror) = ivlapes_impl(
        nlon,
    ...
    let v = expand_ivlapes_output(v, nlat, nlon, nt, ityp);
    let w = expand_ivlapes_output(w, nlat, nlon, nt, ityp);
