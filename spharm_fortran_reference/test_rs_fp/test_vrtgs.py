...
def run_case(nlat: int, nlon: int, nt: int, isym: int = 0):
    ...
    _br, _bi, cr, ci, ierr1 = fort_sp.vhags(v, w, np.asarray(wvhags, dtype=np.float32), lwork_vhags)
    ...
    l1 = min(nlat, (nlon + 2) // 2)
    if isym == 0:
        lwork_vrtgs = nlat * ((nt + 1) * nlon + 2 * nt * l1 + 1)
    else:
        lwork_vrtgs = (nt + 1) * l2 * nlon + nlat * (2 * nt * l1 + 1)
    vort_f, ierr_f = fort_sp.vrtgs(nlon, cr, ci, np.asarray(wshsgs, dtype=np.float32), lwork_vrtgs, isym=isym)
    vort_r, ierr_r = rust_sp.vrtgs(nlon, cr, ci, np.asarray(wshsgs, dtype=np.float32), lwork_vrtgs, isym=isym)
    assert ierr_f == 0, ("fortran vrtgs failed", nlat, nlon, nt, isym, ierr_f)
    assert ierr_r == 0, ("rust vrtgs failed", nlat, nlon, nt, isym, ierr_r)
    summarize_diff("vrtgs vort", vort_f, vort_r)
