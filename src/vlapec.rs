...
pub fn vlapec_impl(
    ...
    let lwkmin = if ityp <= 2 {
        nlat * (2 * nt * nlon + (6 * l2).max(nlon) + 1) + 4 * mn
    } else {
        l2 * (2 * nt * nlon + (6 * nlat).max(nlon)) + 4 * mn + nlat
    };
    ierror = 10;
    if lwork < lwkmin {
        return Ok((Vec::new(), Vec::new(), ierror));
...
pub fn vlapec<'py>(
    ...
    vlapec_ityp(py, nlon, br, bi, cr, ci, 0, wvhsec, lwork)
}
...
pub fn vlapec_ityp<'py>(
    ...
    if shape.len() != 2 && shape.len() != 3 {
        return Err(PyValueError::new_err(
            "vlapec expects rank-2 or rank-3 coefficient arrays",
        ));
    }

    let nlat = shape[0];
    let nt = if shape.len() == 2 { 1 } else { shape[2] };
    let (vlap, wlap, ierror) = vlapec_impl(
        nlon,
