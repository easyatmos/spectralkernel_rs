...
c     if(lvhsgc .lt. 2*(lzz1+labc)+nlon+15) return
     ...
     1   ndab,br,bi,cr,ci,idv,ve,vo,we,wo,vb,wb,wvbin,wwbin,wrfft)
      integer dbgunit, jmax, prow1, prow2, prow3, prow4
      logical dbg
     ...
     4          vb(imid,nlat,3),wb(imid,nlat,3)
      dbgunit = 99
      dbg = nlat.eq.73 .and. nlon.eq.144 .and. nt.eq.1
      jmax = min0(nlon,8)
      prow1 = 1
      prow2 = min0(23,idv)
      prow3 = min0(imid,idv)
      prow4 = min0(imid+1,idv)

      if (dbg) then
         open(unit=dbgunit,file='vhsgc_fortran_trace.txt',
     1        status='unknown',position='append')
         write(dbgunit,*) '--- ENTER VHSGC1 ---'
         write(dbgunit,*) 'nlat=', nlat, ' nlon=', nlon,
     1                    ' nt=', nt, ' ityp=', ityp
         close(dbgunit)
      endif
      nlp1 = nlat+1
...
c
   ...
   16 continue
     ...
     1        status='unknown',position='append')
         ...
         write(dbgunit,*) 'FORTRAN after-m0 ve row1 first 8:'
         write(dbgunit,'(8f14.6)') (ve(prow1,j,1), j=1,jmax)
         write(dbgunit,*) 'FORTRAN after-m0 vo row1 first 8:'
         write(dbgunit,'(8f14.6)') (vo(prow1,j,1), j=1,jmax)
         write(dbgunit,*) 'FORTRAN after-m0 we row1 first 8:'
         write(dbgunit,'(8f14.6)') (we(prow1,j,1), j=1,jmax)
         write(dbgunit,*) 'FORTRAN after-m0 wo row1 first 8:'
         write(dbgunit,'(8f14.6)') (wo(prow1,j,1), j=1,jmax)

         write(dbgunit,*) 'FORTRAN after-m0 ve row23 first 8:'
         write(dbgunit,'(8f14.6)') (ve(prow2,j,1), j=1,jmax)
         write(dbgunit,*) 'FORTRAN after-m0 vo row23 first 8:'
         write(dbgunit,'(8f14.6)') (vo(prow2,j,1), j=1,jmax)
         write(dbgunit,*) 'FORTRAN after-m0 we row23 first 8:'
         write(dbgunit,'(8f14.6)') (we(prow2,j,1), j=1,jmax)
         write(dbgunit,*) 'FORTRAN after-m0 wo row23 first 8:'
         write(dbgunit,'(8f14.6)') (wo(prow2,j,1), j=1,jmax)

         write(dbgunit,*) 'FORTRAN after-m0 ve row37 first 8:'
         write(dbgunit,'(8f14.6)') (ve(prow3,j,1), j=1,jmax)
         write(dbgunit,*) 'FORTRAN after-m0 vo row37 first 8:'
         write(dbgunit,'(8f14.6)') (vo(prow3,j,1), j=1,jmax)
         write(dbgunit,*) 'FORTRAN after-m0 we row37 first 8:'
         write(dbgunit,'(8f14.6)') (we(prow3,j,1), j=1,jmax)
         write(dbgunit,*) 'FORTRAN after-m0 wo row37 first 8:'
         write(dbgunit,'(8f14.6)') (wo(prow3,j,1), j=1,jmax)

         if (prow4 .gt. prow3) then
            write(dbgunit,*) 'FORTRAN after-m0 ve row38 first 8:'
            write(dbgunit,'(8f14.6)') (ve(prow4,j,1), j=1,jmax)
            write(dbgunit,*) 'FORTRAN after-m0 vo row38 first 8:'
            write(dbgunit,'(8f14.6)') (vo(prow4,j,1), j=1,jmax)
            write(dbgunit,*) 'FORTRAN after-m0 we row38 first 8:'
            write(dbgunit,'(8f14.6)') (we(prow4,j,1), j=1,jmax)
            write(dbgunit,*) 'FORTRAN after-m0 wo row38 first 8:'
            write(dbgunit,'(8f14.6)') (wo(prow4,j,1), j=1,jmax)
         endif
...
c
   ...
   30 continue
      ...
      if (dbg .and. (mp1.eq.2 .or. mp1.eq.3 .or. mp1.eq.5)) then
         open(unit=dbgunit,file='vhsgc_fortran_trace.txt',
     1        status='unknown',position='append')

         if (mp1 .eq. 2) then
            write(dbgunit,*) 'FORTRAN after-mp1-2 ve row1 first 8:'
         else if (mp1 .eq. 3) then
            write(dbgunit,*) 'FORTRAN after-mp1-3 ve row1 first 8:'
         else
            write(dbgunit,*) 'FORTRAN after-mp1-5 ve row1 first 8:'
         endif
         write(dbgunit,'(8f14.6)') (ve(prow1,j,1), j=1,jmax)

         if (mp1 .eq. 2) then
            write(dbgunit,*) 'FORTRAN after-mp1-2 vo row1 first 8:'
         else if (mp1 .eq. 3) then
            write(dbgunit,*) 'FORTRAN after-mp1-3 vo row1 first 8:'
         else
            write(dbgunit,*) 'FORTRAN after-mp1-5 vo row1 first 8:'
         endif
         write(dbgunit,'(8f14.6)') (vo(prow1,j,1), j=1,jmax)

         if (mp1 .eq. 2) then
            write(dbgunit,*) 'FORTRAN after-mp1-2 we row1 first 8:'
         else if (mp1 .eq. 3) then
            write(dbgunit,*) 'FORTRAN after-mp1-3 we row1 first 8:'
         else
            write(dbgunit,*) 'FORTRAN after-mp1-5 we row1 first 8:'
         endif
         write(dbgunit,'(8f14.6)') (we(prow1,j,1), j=1,jmax)

         if (mp1 .eq. 2) then
            write(dbgunit,*) 'FORTRAN after-mp1-2 wo row1 first 8:'
         else if (mp1 .eq. 3) then
            write(dbgunit,*) 'FORTRAN after-mp1-3 wo row1 first 8:'
         else
            write(dbgunit,*) 'FORTRAN after-mp1-5 wo row1 first 8:'
         endif
         write(dbgunit,'(8f14.6)') (wo(prow1,j,1), j=1,jmax)
...
c
  ...
  950 continue
     ...
     1        status='unknown',position='append')
         ...
         write(dbgunit,*) 'FORTRAN pre-hrfftb ve row1 first 8:'
         write(dbgunit,'(8f14.6)') (ve(prow1,j,1), j=1,jmax)
         write(dbgunit,*) 'FORTRAN pre-hrfftb ve row23 first 8:'
         write(dbgunit,'(8f14.6)') (ve(prow2,j,1), j=1,jmax)
         write(dbgunit,*) 'FORTRAN pre-hrfftb ve row37 first 8:'
         write(dbgunit,'(8f14.6)') (ve(prow3,j,1), j=1,jmax)
         if (prow4 .gt. prow3) then
            write(dbgunit,*) 'FORTRAN pre-hrfftb ve row38 first 8:'
            write(dbgunit,'(8f14.6)') (ve(prow4,j,1), j=1,jmax)
         endif

         write(dbgunit,*) 'FORTRAN pre-hrfftb we row1 first 8:'
         write(dbgunit,'(8f14.6)') (we(prow1,j,1), j=1,jmax)
         write(dbgunit,*) 'FORTRAN pre-hrfftb we row23 first 8:'
         write(dbgunit,'(8f14.6)') (we(prow2,j,1), j=1,jmax)
         write(dbgunit,*) 'FORTRAN pre-hrfftb we row37 first 8:'
         write(dbgunit,'(8f14.6)') (we(prow3,j,1), j=1,jmax)
         if (prow4 .gt. prow3) then
            write(dbgunit,*) 'FORTRAN pre-hrfftb we row38 first 8:'
            write(dbgunit,'(8f14.6)') (we(prow4,j,1), j=1,jmax)
         endif
         close(dbgunit)
      endif
   ...
   14 continue
     ...
     1        status='unknown',position='append')
         ...
         write(dbgunit,*) 'FORTRAN post-hrfftb ve row1 first 8:'
         write(dbgunit,'(8f14.6)') (ve(prow1,j,1), j=1,jmax)
         write(dbgunit,*) 'FORTRAN post-hrfftb ve row23 first 8:'
         write(dbgunit,'(8f14.6)') (ve(prow2,j,1), j=1,jmax)
         write(dbgunit,*) 'FORTRAN post-hrfftb ve row37 first 8:'
         write(dbgunit,'(8f14.6)') (ve(prow3,j,1), j=1,jmax)
         if (prow4 .gt. prow3) then
            write(dbgunit,*) 'FORTRAN post-hrfftb ve row38 first 8:'
            write(dbgunit,'(8f14.6)') (ve(prow4,j,1), j=1,jmax)
         endif

         write(dbgunit,*) 'FORTRAN post-hrfftb we row1 first 8:'
         write(dbgunit,'(8f14.6)') (we(prow1,j,1), j=1,jmax)
         write(dbgunit,*) 'FORTRAN post-hrfftb we row23 first 8:'
         write(dbgunit,'(8f14.6)') (we(prow2,j,1), j=1,jmax)
         write(dbgunit,*) 'FORTRAN post-hrfftb we row37 first 8:'
         write(dbgunit,'(8f14.6)') (we(prow3,j,1), j=1,jmax)
         if (prow4 .gt. prow3) then
            write(dbgunit,*) 'FORTRAN post-hrfftb we row38 first 8:'
            write(dbgunit,'(8f14.6)') (we(prow4,j,1), j=1,jmax)
         endif
         close(dbgunit)
      endif
   ...
   65 continue
      ...
      subroutine vhsgc_dbg_stage(label, ve, vo, we, wo, idv, nlon,
     1     prow1, prow2, prow3, prow4, jmax, dbgunit)
      character*(*) label
      dimension ve(idv,nlon,1), vo(idv,nlon,1), we(idv,nlon,1),
     1          wo(idv,nlon,1)

      open(unit=dbgunit,file='vhsgc_fortran_trace.txt',
     1     status='unknown',position='append')

      write(dbgunit,*) 'FORTRAN ', label, ' ve row1 first 8:'
      write(dbgunit,'(8f14.6)') (ve(prow1,j,1), j=1,jmax)
      write(dbgunit,*) 'FORTRAN ', label, ' vo row1 first 8:'
      write(dbgunit,'(8f14.6)') (vo(prow1,j,1), j=1,jmax)
      write(dbgunit,*) 'FORTRAN ', label, ' we row1 first 8:'
      write(dbgunit,'(8f14.6)') (we(prow1,j,1), j=1,jmax)
      write(dbgunit,*) 'FORTRAN ', label, ' wo row1 first 8:'
      write(dbgunit,'(8f14.6)') (wo(prow1,j,1), j=1,jmax)

      write(dbgunit,*) 'FORTRAN ', label, ' ve row23 first 8:'
      write(dbgunit,'(8f14.6)') (ve(prow2,j,1), j=1,jmax)
      write(dbgunit,*) 'FORTRAN ', label, ' vo row23 first 8:'
      write(dbgunit,'(8f14.6)') (vo(prow2,j,1), j=1,jmax)
      write(dbgunit,*) 'FORTRAN ', label, ' we row23 first 8:'
      write(dbgunit,'(8f14.6)') (we(prow2,j,1), j=1,jmax)
      write(dbgunit,*) 'FORTRAN ', label, ' wo row23 first 8:'
      write(dbgunit,'(8f14.6)') (wo(prow2,j,1), j=1,jmax)

      write(dbgunit,*) 'FORTRAN ', label, ' ve row37 first 8:'
      write(dbgunit,'(8f14.6)') (ve(prow3,j,1), j=1,jmax)
      write(dbgunit,*) 'FORTRAN ', label, ' vo row37 first 8:'
      write(dbgunit,'(8f14.6)') (vo(prow3,j,1), j=1,jmax)
      write(dbgunit,*) 'FORTRAN ', label, ' we row37 first 8:'
      write(dbgunit,'(8f14.6)') (we(prow3,j,1), j=1,jmax)
      write(dbgunit,*) 'FORTRAN ', label, ' wo row37 first 8:'
      write(dbgunit,'(8f14.6)') (wo(prow3,j,1), j=1,jmax)

      if (prow4 .gt. prow3) then
         write(dbgunit,*) 'FORTRAN ', label, ' ve row38 first 8:'
         write(dbgunit,'(8f14.6)') (ve(prow4,j,1), j=1,jmax)
         write(dbgunit,*) 'FORTRAN ', label, ' vo row38 first 8:'
         write(dbgunit,'(8f14.6)') (vo(prow4,j,1), j=1,jmax)
         write(dbgunit,*) 'FORTRAN ', label, ' we row38 first 8:'
         write(dbgunit,'(8f14.6)') (we(prow4,j,1), j=1,jmax)
         write(dbgunit,*) 'FORTRAN ', label, ' wo row38 first 8:'
         write(dbgunit,'(8f14.6)') (wo(prow4,j,1), j=1,jmax)
      endif
