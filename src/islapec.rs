...
pub fn islapec_impl(
    ...
    let l2 = imid;
    let lwmin = 2 * nlat * l2
        + 3 * (l1_saved.saturating_sub(2) * (2 * nlat - l1_saved - 1)) / 2
        + nlon
        + 15;
    ierror = 9;
    if wshsec.len() < lwmin {
    ...
    let (sf, ierr) = shsec_impl(&as_, &bs_, nlat, nt, 0, wshsec, lwork_shsec)?;
...
pub fn islapec<'py>(
    ...
    if ashape.len() != 2 && ashape.len() != 3 {
        return Err(PyValueError::new_err(
            "islapec expects rank-2 or rank-3 a/b",
        ));
    }
    ...
    let (sf, pertrb, ierror) = islapec_impl(
        nlon,
        xlmbda.as_slice()?,
        &abuf,
        &bbuf,
...
pub fn islapec_isym<'py>(
    ...
    if ashape.len() != 2 && ashape.len() != 3 {
        return Err(PyValueError::new_err(
            "islapec_isym expects rank-2 or rank-3 a/b",
        ));
    }
