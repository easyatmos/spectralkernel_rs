...
def run_case(nlat: int, nlon: int, nt: int):
    ...
    wshaec, ierr0 = rust_sp.shaeci(nlat, nlon, lshaec, ldwork)
    assert ierr0 == 0
    g = make_grid(nlat, nlon, nt)
    n2 = (nlat + 1) // 2
    lwork = nlat * (nt * nlon + max(3 * n2, nlon))
    a_f, b_f, ierr_f = fort_sp.shaec(g, np.asarray(wshaec, dtype=np.float32), lwork)
    a_r, b_r, ierr_r = rust_sp.shaec(g, np.asarray(wshaec, dtype=np.float32), lwork)
    print(f"\n{'=' * 80}\nshaec: nlat={nlat}, nlon={nlon}, nt={nt}\n{'=' * 80}")
    print("ierror fortran =", ierr_f)
    print("ierror rust    =", ierr_r)
    summarize_diff("shaec a", a_f, a_r)
    summarize_diff("shaec b", b_f, b_r)
