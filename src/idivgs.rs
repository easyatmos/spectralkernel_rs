...
/// Core Rust implementation of `idivgs`.
///
/// # Parameters
...
pub fn idivgs_impl(
    ...
    wvhsgs: &[f32],
    lwork: usize,
) -> PyResult<(Vec<f32>, Vec<f32>, Vec<f32>, usize, usize, i32)> {
    let mut ierror = 1;
    if nlat < 3 {
        return Ok((Vec::new(), Vec::new(), Vec::new(), 0, 0, ierror));
    }
    ...
    let l2 = imid;
    let lwmin = l1 * l2 * (2 * nlat - l1 + 1) + nlon + 15;
    ierror = 9;
    if wvhsgs.len() < lwmin {
        return Ok((Vec::new(), Vec::new(), Vec::new(), 0, 0, ierror));
    }
    ...
    let (v, w, idvw, nlon2, ierr) =
        vhsgs_impl(&br, &bi, &cr, &ci, nlat, nt, ityp, wvhsgs, lwork_vhsgs)?;
    Ok((v, w, pertrb, idvw, nlon2, ierr))
}

#[pyfunction]
/// Rust entry point for `idivgs`.
///
/// # Parameters
/// - `a`: Cosine spectral coefficients in scalar layout.
/// - `b`: Sine spectral coefficients in scalar layout.
...
pub fn idivgs<'py>(
    ...
    if ashape.len() != 2 && ashape.len() != 3 {
        return Err(PyValueError::new_err("idivgs expects rank-2 or rank-3 a/b"));
    }
    ...
    let (v, w, pertrb, _idvw, _nlon, ierror) =
        idivgs_impl(&abuf, &bbuf, nlat, nt, 0, wvhsgs.as_slice()?, lwork)?;
    if ierror != 0 {
...
#[pyfunction]
/// Rust entry point for `idivgs_isym`.
///
/// # Parameters
/// - `a`: Cosine spectral coefficients in scalar layout.
/// - `b`: Sine spectral coefficients in scalar layout.
...
pub fn idivgs_isym<'py>(
    ...
    if ashape.len() != 2 && ashape.len() != 3 {
        return Err(PyValueError::new_err(
            "idivgs_isym expects rank-2 or rank-3 a/b",
        ));
    }
    ...
    let abuf = collect_logical_ab(a.as_array());
    let bbuf = collect_logical_ab(b.as_array());
    let (v, w, pertrb, _idv, _nlon, ierror) =
        idivgs_impl(&abuf, &bbuf, nlat, nt, isym, wvhsgs.as_slice()?, lwork)?;
    if ierror != 0 {
        let empty2 = ArrayD::<f32>::from_shape_vec(IxDyn(&[0, 0]), Vec::new())
            .map_err(|e| PyValueError::new_err(e.to_string()))?;
        let empty1 = ArrayD::<f32>::from_shape_vec(IxDyn(&[0]), Vec::new())
