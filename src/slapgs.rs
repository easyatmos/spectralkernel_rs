...
/// Core Rust implementation of `slapgs`.
///
/// # Parameters
...
pub fn slapgs_impl(
    ...
    let lwkmin = if isym == 0 {
        (nt + 1) * nlat * nlon + nlat * (2 * nt * mmax + 1)
    } else {
        (nt + 1) * l2 * nlon + nlat * (2 * nt * mmax + 1)
    };
    ierror = 10;
    if lwork < lwkmin {
        return Ok((Vec::new(), ierror));
    ...
    shsgs_impl(&alap, &blap, nlat, nt, wshsgs, lwork_shsgs)
}

#[pyfunction]
/// Rust entry point for `slapgs`.
///
/// # Parameters
/// - `nlon`: Number of longitudes in the grid.
/// - `a`: Cosine spectral coefficients in scalar layout.
...
pub fn slapgs<'py>(
    ...
    if ashape.len() != 2 && ashape.len() != 3 {
        return Err(PyValueError::new_err("slapgs expects rank-2 or rank-3 a/b"));
    }
