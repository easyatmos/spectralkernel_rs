...
/// Core Rust implementation of `ivlapgc`.
///
/// # Parameters
...
pub fn ivlapgc_impl(
    ...
    let lwork_vhsgc = if matches!(ityp, 0 | 3 | 6) {
        lwork.saturating_sub(4 * mn + nlat)
    } else {
        lwork.saturating_sub(2 * mn + nlat)
    };
    let (v, w, _idvw, _nlon2, ierr) = vhsgc_impl(
        &brvw,
        &bivw,
        &crvw,
        &civw,
...
pub fn ivlapgc<'py>(
    ...
    if shape.len() != 2 && shape.len() != 3 {
        return Err(PyValueError::new_err(
            "ivlapgc expects rank-2 or rank-3 coefficient arrays",
        ));
    }
    let nlat = shape[0];
    let nt = if shape.len() == 2 { 1 } else { shape[2] };
    let (v, w, ierror) = ivlapgc_impl(
        nlon,
...
#[pyfunction]
/// Rust entry point for `ivlapgc_ityp`.
///
/// # Parameters
/// - `nlon`: Number of longitudes in the grid.
/// - `br`: First vector coefficient family in vector layout.
...
pub fn ivlapgc_ityp<'py>(
    ...
    if shape.len() != 2 && shape.len() != 3 {
        return Err(PyValueError::new_err(
            "ivlapgc_ityp expects rank-2 or rank-3 coefficient arrays",
        ));
    }
    let nlat = shape[0];
    let nt = if shape.len() == 2 { 1 } else { shape[2] };
    ...
    let v = expand_ivlapgc_output(v, nlat, nlon, nt, ityp);
    let w = expand_ivlapgc_output(w, nlat, nlon, nt, ityp);
    let out_shape = if shape.len() == 2 {
        IxDyn(&[nlat, nlon])
    } else {
        IxDyn(&[nlat, nlon, nt])
