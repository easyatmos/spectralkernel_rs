...
def run_case(nlat: int, nlon: int, nt: int, isym: int = 0):
    ...
    _br, _bi, cr, ci, ierr1 = fort_sp.vhagc(v, w, np.asarray(wvhagc, dtype=np.float32), lwork_vhagc)
    ...
    l1 = min(nlat, (nlon + 2) // 2)
    if isym == 0:
        lwork_vrtgc = nlat * (nt * nlon + max(3 * l2, nlon) + 2 * nt * l1 + 1)
    else:
        lwork_vrtgc = l2 * (nt * nlon + max(3 * nlat, nlon)) + nlat * (2 * nt * l1 + 1)

    vort_f, ierr_f = fort_sp.vrtgc(nlon, cr, ci, np.asarray(wshsgc, dtype=np.float32), lwork_vrtgc, isym=isym)
    vort_r, ierr_r = rust_sp.vrtgc(nlon, cr, ci, np.asarray(wshsgc, dtype=np.float32), lwork_vrtgc, isym=isym)

    assert ierr_f == 0, ("fortran vrtgc failed", nlat, nlon, nt, isym, ierr_f)
    assert ierr_r == 0, ("rust vrtgc failed", nlat, nlon, nt, isym, ierr_r)

    print(f"\n{'=' * 80}\nvrtgc: nlat={nlat}, nlon={nlon}, nt={nt}, isym={isym}\n{'=' * 80}")
    summarize_diff("vrtgc vort: fortran vs rust", vort_f, vort_r)
