...
def run_case(nlat: int, nlon: int, nt: int, isym: int = 0):
    ...
    if isym == 0:
        ...
        a, b, ierr1 = fort_sp.shags(vt, np.asarray(wshags, dtype=np.float32), lwork_shags)
    else:
        lwork_shags = l2 * (nt * nlon + max(3 * nlat, nlon))
        a, b, ierr1 = fort_sp.shags(vt, np.asarray(wshags, dtype=np.float32), lwork_shags, isym=isym)
    ...
    lwork_doc_eq0 = max(
        nlat * ((2 * nt + 1) * nlon + 2 * nt * min(nlat, (nlon + 1) // 2) + 1),
        nlat * (2 * nt * nlon + max(6 * ((nlat + 1) // 2), nlon)),
    )
    lwork_doc_ne0 = max(
        (2 * nt + 1) * imid * nlon + nlat * (2 * nt * min(nlat, (nlon + 1) // 2) + 1),
        imid * (2 * nt * nlon + max(6 * nlat, nlon)),
    )

    if isym == 0:
        lwork_ivrtgs = max(lwork_branch_fortran_eq0, lwork_branch_fortran_ne0, lwork_doc_eq0)
        v_f, w_f, p_f, ierr_f = fort_sp.ivrtgs(nlon, a, b, np.asarray(wvhsgs, dtype=np.float32), lwork_ivrtgs)
        v_r, w_r, p_r, ierr_r = rust_sp.ivrtgs(a, b, np.asarray(wvhsgs, dtype=np.float32), lwork_ivrtgs)
    else:
        lwork_ivrtgs = max(lwork_branch_fortran_eq0, lwork_branch_fortran_ne0, lwork_doc_ne0)
        v_f, w_f, p_f, ierr_f = fort_sp.ivrtgs(nlon, a, b, np.asarray(wvhsgs, dtype=np.float32), lwork_ivrtgs, isym=isym)
        v_r, w_r, p_r, ierr_r = rust_sp.ivrtgs_isym(a, b, isym, np.asarray(wvhsgs, dtype=np.float32), lwork_ivrtgs)
    assert ierr_f == 0, ("fortran ivrtgs failed", nlat, nlon, nt, isym, ierr_f)
    assert ierr_r == 0, ("rust ivrtgs failed", nlat, nlon, nt, isym, ierr_r)
    summarize_diff("ivrtgs v", v_f, v_r)
    summarize_diff("ivrtgs w", w_f, w_r)
    summarize_diff("ivrtgs pertrb", p_f, p_r)
