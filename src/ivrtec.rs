...
pub fn ivrtec<'py>(
    ...
    if ashape.len() != 2 && ashape.len() != 3 {
        return Err(PyValueError::new_err("ivrtec expects rank-2 or rank-3 a/b"));
    }
    let nlat = ashape[0];
    ...
    let (v, w, pertrb, ierror) =
        ivrtec_impl(nlon, &abuf, &bbuf, nlat, nt, 0, wvhsec.as_slice()?, lwork)?;
    if ierror != 0 {
...
pub fn ivrtec_isym<'py>(
    ...
    if ashape.len() != 2 && ashape.len() != 3 {
        return Err(PyValueError::new_err(
            "ivrtec_isym expects rank-2 or rank-3 a/b",
        ));
    }
    let nlat = ashape[0];
    let nt = if ashape.len() == 2 { 1 } else { ashape[2] };
    ...
    let abuf = collect_logical_ab(a.as_array());
    let bbuf = collect_logical_ab(b.as_array());
    let (v, w, pertrb, ierror) = ivrtec_impl(
        nlon,
        &abuf,
        &bbuf,
        nlat,
