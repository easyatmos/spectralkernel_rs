...
def run_case(nlat: int, nlon: int, nt: int, isym: int = 0):
    ...
    if isym == 0:
        ...
        a, b, ierr1 = fort_sp.shagc(dv, np.asarray(wshagc, dtype=np.float32), lwork_shagc)
    else:
        lwork_shagc = l2 * (nt * nlon + max(3 * nlat, nlon))
        a, b, ierr1 = fort_sp.shagc(dv, np.asarray(wshagc, dtype=np.float32), lwork_shagc, isym=isym)
    ...
    if isym == 0:
        lwork_idivgc = nlat * (2 * nt * nlon + max(6 * imid, nlon)) + 2 * mn + nlat
        v_f, w_f, p_f, ierr_f = fort_sp.idivgc(nlon, a, b, np.asarray(wvhsgc, dtype=np.float32), lwork_idivgc)
        v_r, w_r, p_r, ierr_r = rust_sp.idivgc(a, b, np.asarray(wvhsgc, dtype=np.float32), lwork_idivgc)
    else:
        lwork_idivgc = imid * (2 * nt * nlon + max(6 * nlat, nlon)) + 2 * mn + nlat
        v_f, w_f, p_f, ierr_f = fort_sp.idivgc(nlon, a, b, np.asarray(wvhsgc, dtype=np.float32), lwork_idivgc, isym=isym)
        v_r, w_r, p_r, ierr_r = rust_sp.idivgc_isym(a, b, isym, np.asarray(wvhsgc, dtype=np.float32), lwork_idivgc)

    assert ierr_f == 0, ("fortran idivgc failed", nlat, nlon, nt, isym, ierr_f)
    assert ierr_r == 0, ("rust idivgc failed", nlat, nlon, nt, isym, ierr_r)
    summarize_diff("idivgc v", v_f, v_r)
    summarize_diff("idivgc w", w_f, w_r)
    summarize_diff("idivgc pertrb", p_f, p_r)
