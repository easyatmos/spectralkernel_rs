...
def run_case(nlat: int, nlon: int, nt: int):
    ...
    wshagc, ierr0 = rust_sp.shagci(nlat, nlon, lshagc, ldwork)
    assert ierr0 == 0
    g = make_grid(nlat, nlon, nt)
    n2 = (nlat + 1) // 2
    lwork = nlat * (nt * nlon + max(3 * n2, nlon))
    a_f, b_f, ierr_f = fort_sp.shagc(g, np.asarray(wshagc, dtype=np.float32), lwork)
    a_r, b_r, ierr_r = rust_sp.shagc(g, np.asarray(wshagc, dtype=np.float32), lwork)
    print(f"\n{'=' * 80}\nshagc: nlat={nlat}, nlon={nlon}, nt={nt}\n{'=' * 80}")
    print("ierror fortran =", ierr_f)
    print("ierror rust    =", ierr_r)
    summarize_diff("shagc a", a_f, a_r)
    summarize_diff("shagc b", b_f, b_r)
