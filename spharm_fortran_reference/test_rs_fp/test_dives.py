...
def run_case(nlat: int, nlon: int, nt: int, isym: int = 0):
    ...
    lwork_vhaes = nlat * (4 * nt * nlon + max(6 * ((nlat + 1) // 2), nlon))
    ...
    l1 = min(nlat, (nlon + 2) // 2)
    if isym == 0:
        lwork_dives = nlat * ((nt + 1) * nlon + 2 * nt * l1 + 1)
    else:
        lwork_dives = (nt + 1) * l2 * nlon + nlat * (2 * nt * l1 + 1)

    dv_f, ierr_f = fort_sp.dives(nlon, br, bi, np.asarray(wshses, dtype=np.float32), lwork_dives, isym=isym)
    dv_r, ierr_r = rust_sp.dives(nlon, br, bi, np.asarray(wshses, dtype=np.float32), lwork_dives, isym=isym)

    assert ierr_f == 0, ("fortran dives failed", nlat, nlon, nt, isym, ierr_f)
    assert ierr_r == 0, ("rust dives failed", nlat, nlon, nt, isym, ierr_r)

    print(f"\n{'=' * 80}\ndives: nlat={nlat}, nlon={nlon}, nt={nt}, isym={isym}\n{'=' * 80}")
    summarize_diff("dives dv: fortran vs rust", dv_f, dv_r)
