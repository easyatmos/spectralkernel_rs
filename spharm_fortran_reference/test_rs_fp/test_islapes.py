...
def run_case(nlat: int, nlon: int, nt: int, isym: int = 0):
    ...
    if isym == 0:
        ...
        a, b, ierr1 = fort_sp.shaes(sf_true, np.asarray(wshaes, dtype=np.float32), lwork_shaes)
    else:
        lwork_shaes = l2 * (2 * nt * nlon + max(6 * nlat, nlon))
        a, b, ierr1 = fort_sp.shaes(sf_true, np.asarray(wshaes, dtype=np.float32), lwork_shaes, isym=isym)
    ...
    if isym == 0:
        lwork_islapes = (nt + 1) * nlat * nlon + nlat * (2 * nt * l1 + 1)
        sf_f, p_f, ierr_f = fort_sp.islapes(nlon, xlmbda, a, b, np.asarray(wshses, dtype=np.float32), lwork_islapes)
        sf_r, p_r, ierr_r = rust_sp.islapes(nlon, xlmbda, a, b, np.asarray(wshses, dtype=np.float32), lwork_islapes)
    else:
        lwork_islapes = (nt + 1) * l2 * nlon + nlat * (2 * nt * l1 + 1)
        sf_f, p_f, ierr_f = fort_sp.islapes(nlon, xlmbda, a, b, np.asarray(wshses, dtype=np.float32), lwork_islapes, isym=isym)
        sf_r, p_r, ierr_r = rust_sp.islapes_isym(nlon, xlmbda, a, b, isym, np.asarray(wshses, dtype=np.float32), lwork_islapes)

    assert ierr_f == 0, ("fortran islapes failed", nlat, nlon, nt, isym, ierr_f)
    assert ierr_r == 0, ("rust islapes failed", nlat, nlon, nt, isym, ierr_r)
    summarize_diff("islapes sf", sf_f, sf_r)
    summarize_diff("islapes pertrb", p_f, p_r)
