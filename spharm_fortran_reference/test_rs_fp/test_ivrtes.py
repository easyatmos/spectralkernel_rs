...
def run_case(nlat: int, nlon: int, nt: int, isym: int = 0):
    ...
    if isym == 0:
        ...
        a, b, ierr1 = fort_sp.shaes(vort, np.asarray(wshaes, dtype=np.float32), lwork_shaes)
    else:
        lwork_shaes = l2 * (2 * nt * nlon + max(6 * nlat, nlon))
        a, b, ierr1 = fort_sp.shaes(vort, np.asarray(wshaes, dtype=np.float32), lwork_shaes, isym=isym)
    assert ierr1 == 0, ("shaes failed", nlat, nlon, nt, isym, ierr1)
    lvhses = calc_vhses_sizes(nlat, nlon)
    init_lwork_vhsesi = 3 * max(min(nlat, (nlon + 1) // 2) - 2, 0) * (2 * nlat - min(nlat, (nlon + 1) // 2) - 1) // 2 + 5 * ((nlat + 1) // 2) * nlat
    ldwork_vhsesi = 2 * (nlat + 1)
    ...
    mn = mmax * nlat * nt
    imid = (nlat + 1) // 2
    lwork_eq0 = imid * (2 * nt * nlon + max(6 * nlat, nlon)) + 2 * mn + nlat
    lwork_ne0 = nlat * (2 * nt * nlon + max(6 * imid, nlon)) + 2 * mn + nlat
    if isym == 0:
        lwork_ivrtes = max(lwork_eq0, lwork_ne0)
        v_f, w_f, p_f, ierr_f = fort_sp.ivrtes(nlon, a, b, np.asarray(wvhses, dtype=np.float32), lwork_ivrtes)
        v_r, w_r, p_r, ierr_r = rust_sp.ivrtes(a, b, np.asarray(wvhses, dtype=np.float32), lwork_ivrtes)
    else:
        lwork_ivrtes = max(lwork_eq0, lwork_ne0)
        v_f, w_f, p_f, ierr_f = fort_sp.ivrtes(nlon, a, b, np.asarray(wvhses, dtype=np.float32), lwork_ivrtes, isym=isym)
        v_r, w_r, p_r, ierr_r = rust_sp.ivrtes_isym(a, b, isym, np.asarray(wvhses, dtype=np.float32), lwork_ivrtes)
    assert ierr_f == 0, ("fortran ivrtes failed", nlat, nlon, nt, isym, ierr_f)
    assert ierr_r == 0, ("rust ivrtes failed", nlat, nlon, nt, isym, ierr_r)
    summarize_diff("ivrtes v", v_f, v_r)
    summarize_diff("ivrtes w", w_f, w_r)
    summarize_diff("ivrtes pertrb", p_f, p_r)
