...
def run_case(nlat: int, nlon: int, nt: int, isym: int = 0):
    ...
    lshags, init_lwork_shagsi, ldwork_shagsi = calc_shags_sizes(nlat, nlon)
    wshags, ierr0 = fort_sp.shagsi(nlat, nlon, lshags, init_lwork_shagsi, ldwork_shagsi)
    assert ierr0 == 0, ("shagsi failed", nlat, nlon, ierr0)

    l2 = (nlat + 1) // 2
    if isym == 0:
        lwork_shags = nlat * (nt * nlon + max(3 * l2, nlon))
        a, b, ierr1 = fort_sp.shags(sf_true, np.asarray(wshags, dtype=np.float32), lwork_shags)
    else:
        lwork_shags = l2 * (nt * nlon + max(3 * nlat, nlon))
        a, b, ierr1 = fort_sp.shags(sf_true, np.asarray(wshags, dtype=np.float32), lwork_shags, isym=isym)
    ...
    lshsgs, init_lwork_shsgsi, ldwork_shsgsi = calc_shsgs_sizes(nlat, nlon)
    ...
    if isym == 0:
        lwork_islapgs = (nt + 1) * nlat * nlon + nlat * (2 * nt * l1 + 1)
        sf_f, p_f, ierr_f = fort_sp.islapgs(nlon, xlmbda, a, b, np.asarray(wshsgs, dtype=np.float32), lwork_islapgs)
        sf_r, p_r, ierr_r = rust_sp.islapgs(nlon, xlmbda, a, b, np.asarray(wshsgs, dtype=np.float32), lwork_islapgs)
    else:
        lwork_islapgs = (nt + 1) * l2 * nlon + nlat * (2 * nt * l1 + 1)
        sf_f, p_f, ierr_f = fort_sp.islapgs(nlon, xlmbda, a, b, np.asarray(wshsgs, dtype=np.float32), lwork_islapgs, isym=isym)
        sf_r, p_r, ierr_r = rust_sp.islapgs_isym(nlon, xlmbda, a, b, isym, np.asarray(wshsgs, dtype=np.float32), lwork_islapgs)

    assert ierr_f == 0, ("fortran islapgs failed", nlat, nlon, nt, isym, ierr_f)
    assert ierr_r == 0, ("rust islapgs failed", nlat, nlon, nt, isym, ierr_r)
    summarize_diff("islapgs sf", sf_f, sf_r)
    summarize_diff("islapgs pertrb", p_f, p_r)
