...
def run_case(nlat: int, nlon: int, nt: int):
    lshaes, init_lwork, ldwork = calc_shaesi_sizes(nlat, nlon)
    wshaes, ierr0 = rust_sp.shaesi(nlat, nlon, lshaes, init_lwork, ldwork)
    assert ierr0 == 0
    g = make_grid(nlat, nlon, nt)
    lwork = (nt + 1) * nlat * nlon
    a_f, b_f, ierr_f = fort_sp.shaes(g, np.asarray(wshaes, dtype=np.float32), lwork)
    a_r, b_r, ierr_r = rust_sp.shaes(g, np.asarray(wshaes, dtype=np.float32), lwork)
    print(f"\n{'=' * 80}\nshaes: nlat={nlat}, nlon={nlon}, nt={nt}\n{'=' * 80}")
    print("ierror fortran =", ierr_f)
    print("ierror rust    =", ierr_r)
    summarize_diff("shaes a", a_f, a_r)
    summarize_diff("shaes b", b_f, b_r)
