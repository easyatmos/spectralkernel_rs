...
def run_case(nlat: int, nlon: int, nt: int, isym: int = 0):
    ...
    if isym == 0:
        ...
        a, b, ierr1 = fort_sp.shagc(sf, np.asarray(wshagc, dtype=np.float32), lwork_shagc)
    else:
        lwork_shagc = l2 * (nt * nlon + max(3 * nlat, nlon))
        a, b, ierr1 = fort_sp.shagc(sf, np.asarray(wshagc, dtype=np.float32), lwork_shagc, isym=isym)
    ...
    l1 = min(nlat, (nlon + 2) // 2)
    if isym == 0:
        lwork_slapgc = nlat * (2 * nt * nlon + max(6 * l2, nlon) + 2 * nt * l1 + 1)
    else:
        lwork_slapgc = l2 * (2 * nt * nlon + max(6 * nlat, nlon)) + nlat * (2 * nt * l1 + 1)
    slap_f, ierr_f = fort_sp.slapgc(nlon, a, b, np.asarray(wshsgc, dtype=np.float32), lwork_slapgc, isym=isym)
    slap_r, ierr_r = rust_sp.slapgc(nlon, a, b, np.asarray(wshsgc, dtype=np.float32), lwork_slapgc, isym=isym)
    assert ierr_f == 0, ("fortran slapgc failed", nlat, nlon, nt, isym, ierr_f)
    assert ierr_r == 0, ("rust slapgc failed", nlat, nlon, nt, isym, ierr_r)
    summarize_diff("slapgc slap", slap_f, slap_r)
