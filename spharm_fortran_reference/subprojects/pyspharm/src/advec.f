...
c
      ...
      lambdc = omega*t
      call advec_stoc(1.,beta,lambdc,xc,yc,zc)
      ca = cos(alpha)
      sa = sin(alpha)
      pi = 4.*atan(1.)
      tpi = pi+pi
      ...
      cth = clh*cthclh+slh*cthslh
      that = atanxy(sth,cth)
      call advec_stoc(1.,that,lhat,x1,y1,z1)
 ...
 10   continue
      ...
      subroutine advec_ctos(x,y,z,r,theta,phi)
      r1 = x*x+y*y
      if(r1 .ne. 0.) go to 10
      phi = 0.
      theta = 0.
   ...
   10 r = sqrt(r1+z*z)
      ...
      subroutine advec_stoc(r,theta,phi,x,y,z)
      st = sin(theta)
      x = r*st*cos(phi)
      y = r*st*sin(phi)
      z = r*cos(theta)
