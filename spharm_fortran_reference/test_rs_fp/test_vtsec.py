...
def run_case(nlat: int, nlon: int, nt: int, ityp: int = 0):
    ...
    wvts_f, ierr0_f = fort_sp.vtseci(nlat, nlon, lwvts, ldwork)
    wvts_r, ierr0_r = rust_sp.vtseci(nlat, nlon, lwvts, ldwork)
    assert ierr0_f == 0, ("fortran vtseci failed", nlat, nlon, lwvts, ldwork, ierr0_f)
    assert ierr0_r == 0, ("rust vtseci failed", nlat, nlon, lwvts, ldwork, ierr0_r)
    ...
    if ityp == 0:
        vt_f, wt_f, ierr_f = fort_sp.vtsec(nlon, br, bi, cr, ci, wvts_f, lwork)
        vt_r, wt_r, ierr_r = rust_sp.vtsec(br, bi, cr, ci, wvts_r, lwork)
        vt_rf, wt_rf, _ = rust_sp.vtsec(br, bi, cr, ci, wvts_f, lwork)
        vt_fr, wt_fr, _ = fort_sp.vtsec(nlon, br, bi, cr, ci, wvts_r, lwork)
    else:
        vt_f, wt_f, ierr_f = fort_sp.vtsec(nlon, br, bi, cr, ci, wvts_f, lwork, ityp=ityp)
        vt_r, wt_r, ierr_r = rust_sp.vtsec_ityp(br, bi, cr, ci, ityp, wvts_r, lwork)
        vt_rf, wt_rf, _ = rust_sp.vtsec_ityp(br, bi, cr, ci, ityp, wvts_f, lwork)
        vt_fr, wt_fr, _ = fort_sp.vtsec(nlon, br, bi, cr, ci, wvts_r, lwork, ityp=ityp)

    print(f"\n{'=' * 80}\nvtsec: nlat={nlat}, nlon={nlon}, nt={nt}, ityp={ityp}\n{'=' * 80}")
    print("ierror init fortran =", ierr0_f)
    print("ierror init rust    =", ierr0_r)
    print("ierror fortran      =", ierr_f)
    print("ierror rust         =", ierr_r)
    summarize_diff("vtsec init", wvts_f, wvts_r)
    summarize_diff("vtsec vt", vt_f, vt_r)
    summarize_diff("vtsec wt", wt_f, wt_r)
    summarize_diff("rust(vtsec, fortran init) vt", vt_f, vt_rf)
    summarize_diff("rust(vtsec, fortran init) wt", wt_f, wt_rf)
    summarize_diff("fortran(vtsec, rust init) vt", vt_f, vt_fr)
    summarize_diff("fortran(vtsec, rust init) wt", wt_f, wt_fr)
