...
def make_vec_grid(nlat: int, nlon: int, nt: int):
    ...
    for k in range(nt):
        v[:, :, k] = (
            ...
            + 0.3 * np.sin(theta)[:, None] * np.sin((k + 2) * lon)[None, :]
        )
        w[:, :, k] = (
            np.sin(theta)[:, None] * np.sin((k + 1) * lon)[None, :]
            + 0.2 * np.cos(theta)[:, None] * np.cos((k + 3) * lon)[None, :]
        )
    return v, w
...
def run_case(nlat: int, nlon: int, nt: int):
    ...
    l2 = (nlat + 1) // 2
    l1 = min(nlat, (nlon + 2) // 2)
    lwork_igradec = nlat * (nt * nlon + max(3 * l2, nlon) + 2 * nt * l1 + 1)
    sf_f, ierr_f = fort_sp.igradec(nlon, br, bi, np.asarray(wshsec, dtype=np.float32), lwork_igradec)
    sf_r, ierr_r = rust_sp.igradec(br, bi, np.asarray(wshsec, dtype=np.float32), lwork_igradec)

    assert ierr_f == 0, ("fortran igradec failed", nlat, nlon, nt, ierr_f)
    assert ierr_r == 0, ("rust igradec failed", nlat, nlon, nt, ierr_r)

    print(f"\n{'=' * 80}\nigradec: nlat={nlat}, nlon={nlon}, nt={nt}\n{'=' * 80}")
    summarize_diff("igradec sf: fortran vs rust", sf_f, sf_r)
