...
pub fn isfvpgc<'py>(
    ...
    isfvpgc_isym(py, nlon, 0, as_, bs, av, bv, wvhsgc, lwork)
}
...
pub fn isfvpgc_isym<'py>(
    ...
    if shape.len() != 2 && shape.len() != 3 {
        return Err(PyValueError::new_err(
            "isfvpgc expects rank-2 or rank-3 coefficient arrays",
        ));
    }

    let nlat = shape[0];
    let nt = if shape.len() == 2 { 1 } else { shape[2] };
    let (v, w, ierror) = isfvpgc_impl(
        nlon,
    ...
    let v = expand_isfvpgc_output(v, nlat, nlon, nt, isym);
    let w = expand_isfvpgc_output(w, nlat, nlon, nt, isym);
