...
def calc_vhsgs_sizes(nlat: int, nlon: int):
    ...
    lvhsgs = n1 * imid * (2 * nlat - n1 + 1) + nlon + 15 + 2 * nlat
    ...
    for k in range(nt):
        for m in range(mmax):
            for n in range(m, nlat):
                seq = (m * nlat + n) * nt + k
                as_[m, n, k] = 0.06 + 0.0025 * seq
                bs[m, n, k] = -0.04 + 0.0035 * seq
                av[m, n, k] = 0.03 - 0.0018 * seq
                bv[m, n, k] = -0.02 + 0.0012 * seq
    return as_, bs, av, bv
...
def run_case(nlat: int, nlon: int, nt: int, isym: int = 0):
    print(f"\n{'=' * 80}\nisfvpgs: nlat={nlat}, nlon={nlon}, nt={nt}, isym={isym}\n{'=' * 80}")
    ...
    vh_v_f, vh_w_f, vh_ierr_f = fort_sp.vhsgs(nlon, br, bi, cr, ci, wvhsgs, lwork, ityp=ityp)
    vh_v_r, vh_w_r, vh_ierr_r = rust_sp.vhsgs_ityp(br, bi, cr, ci, ityp, wvhsgs, lwork)
    assert vh_ierr_f == 0, ("fortran vhsgs failed", nlat, nlon, nt, isym, vh_ierr_f)
    assert vh_ierr_r == 0, ("rust vhsgs failed", nlat, nlon, nt, isym, vh_ierr_r)
    summarize_diff("isfvpgs low-level vhsgs v", vh_v_f, vh_v_r)
    summarize_diff("isfvpgs low-level vhsgs w", vh_w_f, vh_w_r)

    if isym == 0:
        v_f, w_f, ierr_f = fort_sp.isfvpgs(nlon, as_, bs, av, bv, wvhsgs, lwork)
        v_r, w_r, ierr_r = rust_sp.isfvpgs(nlon, as_, bs, av, bv, wvhsgs, lwork)
    else:
        v_f, w_f, ierr_f = fort_sp.isfvpgs(nlon, as_, bs, av, bv, wvhsgs, lwork, isym=isym)
        v_r, w_r, ierr_r = rust_sp.isfvpgs_isym(nlon, isym, as_, bs, av, bv, wvhsgs, lwork)

    assert ierr_f == 0, ("fortran isfvpgs failed", nlat, nlon, nt, isym, ierr_f)
    assert ierr_r == 0, ("rust isfvpgs failed", nlat, nlon, nt, isym, ierr_r)
    summarize_diff("isfvpgs v", v_f, v_r)
    summarize_diff("isfvpgs w", w_f, w_r)
    summarize_relative_error("isfvpgs v", v_f, v_r)
    summarize_relative_error("isfvpgs w", w_f, w_r)
