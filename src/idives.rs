...
/// Core Rust implementation of `idives`.
///
/// # Parameters
...
pub fn idives<'py>(
    ...
    if ashape.len() != 2 && ashape.len() != 3 {
        return Err(PyValueError::new_err("idives expects rank-2 or rank-3 a/b"));
    }
    let nlat = ashape[0];
    ...
    let (v, w, pertrb, ierror) = idives_impl(&abuf, &bbuf, nlat, nt, 0, wvhses.as_slice()?, lwork)?;
    if ierror != 0 {
...
#[pyfunction]
/// Rust entry point for `idives_isym`.
///
/// # Parameters
/// - `a`: Cosine spectral coefficients in scalar layout.
/// - `b`: Sine spectral coefficients in scalar layout.
...
pub fn idives_isym<'py>(
    ...
    if ashape.len() != 2 && ashape.len() != 3 {
        return Err(PyValueError::new_err(
            "idives_isym expects rank-2 or rank-3 a/b",
        ));
    }
    let nlat = ashape[0];
    let nt = if ashape.len() == 2 { 1 } else { ashape[2] };
    ...
    let (v, w, pertrb, ierror) =
        idives_impl(&abuf, &bbuf, nlat, nt, isym, wvhses.as_slice()?, lwork)?;
    if ierror != 0 {
        let empty2 = ArrayD::<f32>::from_shape_vec(IxDyn(&[0, 0]), Vec::new())
            .map_err(|e| PyValueError::new_err(e.to_string()))?;
        let empty1 = ArrayD::<f32>::from_shape_vec(IxDyn(&[0]), Vec::new())
