    ...
    for k in range(nt):
        for m in range(mmax):
            for n in range(m, nlat):
                seq = (m * nlat + n) * nt + k
                as_[m, n, k] = 0.08 + 0.002 * seq
                bs[m, n, k] = -0.03 + 0.003 * seq
                av[m, n, k] = 0.01 - 0.0015 * seq
                bv[m, n, k] = -0.025 + 0.001 * seq
    return as_, bs, av, bv
...
def run_case(nlat: int, nlon: int, nt: int, isym: int = 0):
    print(f"\n{'=' * 80}\nisfvpec: nlat={nlat}, nlon={nlon}, nt={nt}, isym={isym}\n{'=' * 80}")
    lvhsec, ldwork = calc_vhsec_sizes(nlat, nlon)
    wvhsec, ierr0 = fort_sp.vhseci(nlat, nlon, lvhsec, ldwork)
    assert ierr0 == 0, ("vhseci failed", nlat, nlon, ierr0)

    as_, bs, av, bv = make_coeffs(nlat, nt, nlon)
    lwork = calc_lwork(nlat, nlon, nt, isym)
    wvhsec = np.asarray(wvhsec, dtype=np.float32)

    br, bi, cr, ci = build_vector_coeffs(as_, bs, av, bv, nlat, nlon, nt)
    ityp = {0: 0, 1: 3, 2: 6}[isym]
    vh_v_f, vh_w_f, vh_ierr_f = fort_sp.vhsec(nlon, br, bi, cr, ci, wvhsec, lwork, ityp=ityp)
    vh_v_r, vh_w_r, vh_ierr_r = rust_sp.vhsec_ityp(br, bi, cr, ci, ityp, wvhsec, lwork)
    assert vh_ierr_f == 0, ("fortran vhsec failed", nlat, nlon, nt, isym, vh_ierr_f)
    assert vh_ierr_r == 0, ("rust vhsec failed", nlat, nlon, nt, isym, vh_ierr_r)
    summarize_diff("isfvpec low-level vhsec v", vh_v_f, vh_v_r)
    summarize_diff("isfvpec low-level vhsec w", vh_w_f, vh_w_r)

    if isym == 0:
        v_f, w_f, ierr_f = fort_sp.isfvpec(nlon, as_, bs, av, bv, wvhsec, lwork)
        v_r, w_r, ierr_r = rust_sp.isfvpec(nlon, as_, bs, av, bv, wvhsec, lwork)
    else:
        v_f, w_f, ierr_f = fort_sp.isfvpec(nlon, as_, bs, av, bv, wvhsec, lwork, isym=isym)
        v_r, w_r, ierr_r = rust_sp.isfvpec_isym(nlon, isym, as_, bs, av, bv, wvhsec, lwork)

    assert ierr_f == 0, ("fortran isfvpec failed", nlat, nlon, nt, isym, ierr_f)
    assert ierr_r == 0, ("rust isfvpec failed", nlat, nlon, nt, isym, ierr_r)
    summarize_diff("isfvpec v", v_f, v_r)
    summarize_diff("isfvpec w", w_f, w_r)
    # summarize_relative_error("isfvpec v", v_f, v_r)
    # summarize_relative_error("isfvpec w", w_f, w_r)
